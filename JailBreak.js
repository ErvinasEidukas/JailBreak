...
function RightButtonAction() {
    ...
    RightButtonTimer = setInterval(WhileHoldingRight, 0);
}
...
function BallStartingPosition() {
    ...
    ballSpeed_x = ballSpeed;
    ballSpeed_y = ballSpeed;
    ball_x = platform_x + platform_With / 2 - ballCurrentSize / 2;
    ball_y = gameZone_Height - Platform_height - ballCurrentSize;
...
function WallColision() {
    ...
    if (ball_x >= gameZone_width - ballCurrentSize || ball_x <= 0) {
        if (ball_x >= gameZone_width - ballCurrentSize) {
            ball_x = gameZone_width - ballCurrentSize;
        }
        else {
            ball_x = 0
        }
    ...
    //bottom wall
    if (ball_y + ballCurrentSize >= gameZone_Height) {
        // outoplay
        // ballSpeed_y *= -1;
        // return;
    ...
    for (let i = 0; i < Boxes.length; i++) {
        ...
        if (found) {
            ...
            else if (Boxes[i].classList.contains("Box-Type_blue")) // purple_box
            {
                ...
                ballCurrentSize = ball_size * 2;
                ballStyle.style.width = ballCurrentSize + "px";
                ballStyle.style.height = ballCurrentSize + "px";
...
function StartGame() {
    ...
    // Starting game loop
    GameLoop = setInterval(moveBall, 0);
}
